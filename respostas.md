...
A) O código avalia a expressão booleana, imprime o resultado `false`, calcula a soma dos números de 1 a 5 e imprime o resultado no console. **<-- RESPOSTA CORRETA**
...
```
RESPOSTA{Classe Retangulo estende FormaGeometrica:
    Atributos:
        - largura
        - altura

    Método Construtor(cor, largura, altura):
        Chama o construtor da classe base com cor
...
```
RESPOSTA{
Defina velocidadeInicial, aceleracao, velocidadeMaxima, tempoMaximo, distancia
Defina tempo = 0
Defina distanciaPercorrida = 0

Enquanto tempo <= tempoMaximo E distanciaPercorrida < distancia faça:
    Defina velocidade = velocidadeInicial + aceleracao * tempo
    Se velocidade > velocidadeMaxima então:
        velocidade = velocidadeMaxima
    Fim se
    Defina distanciaPercorrida = velocidade * tempo
    tempo = tempo + 1
Fim enquanto

Se distanciaPercorrida >= distancia então:
    Imprima "O carro completou a corrida em " + tempo + " minutos."
Senão:
    Imprima "O carro não completou a corrida no tempo máximo."
Fim se
}
______
...
Função MultiplicaMatrizes(matrizA, matrizB):
    ...
    Senão:
        Defina linhas = linhas(matrizA)
        Defina colunas = colunas(matrizB)
        Defina matrizResultado = novaMatriz(linhas, colunas)
